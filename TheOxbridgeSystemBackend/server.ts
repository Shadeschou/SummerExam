    ...
    try {
        ...
        if (ships.length > 0) {
            ...
            ships.forEach(async (ship: IShip) => {
                const eventRegistrations: IEventRegistration[] = await EventRegistrationModel.find({ shipId: ship.shipId }, { _id: 0, __v: 0 });
...
app.get('/ships/fromEventId/:eventId', async (req, res) => {
    try {
        const evId: any = req.params.eventId;
        const ships: any[] = [];
        const eventRegistrations: IEventRegistration[] = await EventRegistrationModel.find({ eventId: evId }, { _id: 0, __v: 0 });
...
app.get('/racePoints/findStartAndFinish/:eventId', async (req, res) => {
    try {
        const evId: any = req.params.eventId;
        const racePoints: IRacePoint[] = await RacePointModel.find({ eventId: evId, $or: [{ type: 'startLine' }, { type: 'finishLine' }] }, { _id: 0, __v: 0 });
...
app.get('/racepoints/fromEventId/:eventId', async (req, res) => {
    const evId: any = req.params.eventId;
    const racePoints: IRacePoint[] = await RacePointModel.find({ eventId: evId }, { _id: 0, __v: 0 }, { sort: { racePointNumber: 1 } });
...
app.get('/eventRegistrations/findEventRegFromUsername/:eventId', async (req, res) => {
    ...
    try {
        ...
        shipByEmailUserName.forEach(async (ship: IShip) => {
            ...
            const eventRegistration: IEventRegistration[] = await EventRegistrationModel.find({ eventId: evId, shipId: sId }, { _id: 0, __v: 0 })
            pending--;
...
app.post('/eventRegistrations/signUp', async (req, res) => {
    ...
    try {
        ...
        if (event) {
            ...
            const eventRegistration: IEventRegistration = await EventRegistrationModel.findOne({ shipId: req.body.shipId, eventId: event.eventId }, { _id: 0, __v: 0 });
...
app.post('/eventRegistrations/addParticipant', async (req, res) => {
    ...
    try {
        ...
        const ship: IShip = await ShipModel.findOne({ emailUsername: req.body.emailUsername, name: req.body.shipName }, { _id: 0, __v: 0 });
        if (!ship) {
            ...
            const newEventRegistration = new EventRegistrationModel({ "eventId": req.body.eventId, "shipId": ship.shipId, "trackColor": "Yellow", "teamName": req.body.teamName })
            const regDone: IEventRegistration = await Validate.createRegistration(newEventRegistration, res);
            res.status(201).json(regDone);
        }
...
app.get('/locationRegistrations/getLive/:eventId', async (req, res) => {
    ...
    try {
        ...
        eventRegistrations.forEach(async (eventRegistration: IEventRegistration) => {
            ...
            const locationRegistration: ILocationRegistration[] = await LocationRegistrationModel.find({ eventRegId: eventRegistration.eventRegId }, { _id: 0, __v: 0 }, { sort: { 'locationTime': -1 }, limit: 20 });
    ...
    try {
        ...
        if (eventRegistrations.length !== 0) {
            ...
            eventRegistrations.forEach(async (eventRegistration: IEventRegistration) => {
                ...
                const locationRegistrations: ILocationRegistration[] = await LocationRegistrationModel.find({ eventRegId: eventRegistration.eventRegId }, { _id: 0, __v: 0 }, { sort: { 'locationTime': 1 } });
    ...
    try {
        ...
        if (eventRegistrations.length !== 0) {
            ...
            eventRegistrations.forEach(async (eventReg: IEventRegistration) => {
                ...
                const locationRegistration: ILocationRegistration[] = await LocationRegistrationModel.find({ eventRegId: eventReg.eventRegId }, { _id: 0, __v: 0 }, { sort: { 'locationTime': -1 }, limit: 1 });
                if (locationRegistration.length !== 0) {
