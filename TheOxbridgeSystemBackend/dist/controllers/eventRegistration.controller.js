...
exports.createRegistration = (newRegistration, res, callback) => {
    validateForeignKeys(newRegistration, res, function (err) {
        ...
        eventRegistration_1.EventRegistrationModel.findOne({}).sort('-eventRegId').exec(function (err, lastEventRegistration) {
            if (err)
...
exports.findAll = (req, res) => {
    // Checking if authorized
    authentication_controller_1.Authorize(req, res, "admin", function (err) {
        if (err)
            return err;
        // Finding all the registrations in the db
        eventRegistration_1.EventRegistrationModel.find({}, {_id: 0, __v: 0}, (err, eventRegistrations) => {
            if (err)
...
exports.findEventRegFromUsername = (req, res) => {
    // Checking if authorized
    authentication_controller_1.Authorize(req, res, "user", function (err, decodedUser) {
        if (err)
            return err;
        ...
        ship_1.ShipModel.find({emailUsername: decodedUser.id}, {_id: 0, __v: 0}, null, (err, ships) => {
            ...
            ships.forEach(ship => {
                counter++;
                eventRegistration_1.EventRegistrationModel.find({eventId: req.params.eventId, shipId: ship.shipId}, {
                    _id: 0,
                    __v: 0
...
exports.signUp = (req, res) => {
    ...
    authentication_controller_1.Authorize(req, res, "user", function (err) {
        ...
        event_1.EventModel.findOne({eventCode: req.body.eventCode}, {_id: 0, __v: 0}, null, (err, event) => {
            if (err)
            ...
            if (event) {
                ...
                eventRegistration_1.EventRegistrationModel.findOne({shipId: req.body.shipId, eventId: event.eventId}, {
                    _id: 0,
                    __v: 0
    ...
    eventRegistration_1.EventRegistrationModel.find({eventId: req.params.eventId}, {
        ...
        if (eventRegs !== 0) {
            eventRegs.forEach(eventRegistration => {
                pending++;
                ship_1.ShipModel.findOne({shipId: eventRegistration.shipId}, {_id: 0, __v: 0}, null, (err, ship) => {
                    if (err)
...
exports.addParticipant = (req, res) => {
    ...
    authentication_controller_1.Authorize(req, res, "admin", function (err) {
        ...
        user_1.UserModel.findOne({emailUsername: req.body.emailUsername}, {_id: 0, __v: 0}, (err, user) => {
            if (err)
                return res.status(500).send({message: "error retrieving user"});
            if (!user) {
            ...
            ship_1.ShipModel.findOne({emailUsername: req.body.emailUsername, name: req.body.shipName}, {
                _id: 0,
                __v: 0
            }, (err, ship) => {
                if (err)
                    return res.status(500).send({message: "error retrieving ship"});
                if (!ship) {
                    const newShip = new ship_1.ShipModel({
                        "name": req.body.shipName,
                        "emailUsername": req.body.emailUsername
                    });
                    ship_1.ShipModel.findOne({}).sort('-shipId').exec(function (err, lastShip) {
                        if (err)
                        ...
                        newShip.save(function (err, savedShip) {
                            ...
                            eventRegistration_1.EventRegistrationModel.create(newEventRegistration, res);
                        });
                    });
                } else {
                    const newEventRegistration = new eventRegistration_1.EventRegistrationModel({
...
exports.updateParticipant = (req, res) => {
    ...
    authentication_controller_1.Authorize(req, res, "admin", function (err) {
        if (err)
            return err;
        eventRegistration_1.EventRegistrationModel.findOneAndUpdate({eventRegId: req.params.eventRegId}, req.body, (err, eventReg) => {
            if (err)
                return res.status(500).send({message: "Error updating eventRegistration with eventRegId " + req.params.eventRegId});
            if (eventReg) {
                ship_1.ShipModel.findOneAndUpdate({shipId: eventReg.shipId}, req.body, (err, ship) => {
                    if (err)
                        return res.status(500).send({message: "Error updating ship with shipId " + eventReg.shipId});
                    if (ship) {
                        user_1.UserModel.findOneAndUpdate({emailUsername: ship.emailUsername}, req.body, (err, user) => {
                            if (err)
...
exports.delete = (req, res) => {
    // Checking if authorized
    authentication_controller_1.Authorize(req, res, "all", function (err) {
        if (err)
            return err;
        // Finding and deleting the registration with the given regId
        eventRegistration_1.EventRegistrationModel.findOneAndDelete({eventRegId: req.params.eventRegId}, (err, eventRegistration) => {
            if (err)
...
function validateForeignKeys(registration, res, callback) {
    ...
    ship_1.ShipModel.findOne({shipId: registration.shipId}, function (err, ship) {
        if (err)
        ...
        event_1.EventModel.findOne({eventId: registration.eventId}, function (err, event) {
            if (err)
