...
export class LocationRegistrationService {
  ...
  public getLive(eventId): Observable<Score[]> {
    return this.http.get<Score[]>(this.locationRegUrl + 'getLive/' + eventId).pipe(map(score => score));
  }
  ...
  public getScoreboard(eventId): Observable<Score[]> {
    return this.http.get<Score[]>(this.locationRegUrl + 'getScoreboard/' + eventId).pipe(map(score => score));
  }
}
